...
import { submitOracleAssessment } from "../actions";
import type { ORACLE_QUESTIONS } from "@/lib/claude/oracle";

type Questions = typeof ORACLE_QUESTIONS;

export default function OracleForm({ questions }: { questions: Questions }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        await submitOracleAssessment(formData);
      } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      {questions.map((q, i) => (
        <fieldset key={q.key} className="rounded-lg border border-white/10 bg-white/[0.03] p-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <p className="text-xs text-white/40">
          {isPending ? "ORACLE is analyzing — this may take 10-15 seconds…" : "Powered by Claude Sonnet 4.6"}
        </p>
        <button
          type="submit"
          disabled={isPending}
